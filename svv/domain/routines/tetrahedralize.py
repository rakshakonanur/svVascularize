...
def tetrahedralize(surface: pv.PolyData,
    ...
    # Convert desired max edge length to TetGen max tetra volume (-a)
    if max_edge_length is not None:
        h = float(max_edge_length)
        if h <= 0:
            raise ValueError("max_edge_length must be > 0")

        # Regular tetra volume from edge length (approx)
        import math
        max_vol = (math.sqrt(2.0) / 12.0) * (h ** 3)

        # Most TetGen python wrappers accept either:
        #  - "maxvolume" kwarg, or
        #  - an "a" kwarg, or
        #  - passing "-a" in args
        #
        # Since you pass args/kwargs to a worker, safest is to pass "-a" explicitly:
        tet_args = list(tet_args)
        tet_args += ["-a", str(max_vol)]
        tet_args = tuple(tet_args)
